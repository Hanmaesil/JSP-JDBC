...
<body>
	...
	<div id="wrapper">
		...
		</c:choose> <!-- <a href="#menu">로그인</a> --> <!-- 로그인 후 Logout.jsp로 이동할 수 있는'로그아웃'링크와 '개인정보수정'링크를 출력하시오. -->
		</nav> </header>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="message" items="${msglist}">
									<li>
									<strong>보낸사람 </strong>  ${message.m_sendName} <br>
									<strong>내용 ${message.m_content} </strong><br>
									<strong>날짜 ${message.m_sendDate}</strong><br>
									<a href='MsgDeleteCon?num=${message.m_num}'>삭제</a>
									</li>
								</c:forEach>
